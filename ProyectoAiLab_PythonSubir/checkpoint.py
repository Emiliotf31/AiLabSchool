...
from CPU_ataq_hist import ataq_hp
import subprocess
...
pos_cpu=leer_csv(path=path)[2][3]
...
def comb_entr(pos_per,entrena,ls_pim,ls_decor,ls_habilidades):
    direc="./Registros_usuarios/Usuario_Actual.csv"
    usuario=leer_csv(path=direc)[2][0]
    path="./Registros_usuarios/"+usuario+".csv"
    pos_cpu=int(leer_csv(path=path)[2][3])-1
    pos_per =pos_per-1
    ...
    comb.title("COMBATE (MODO HISTORIA)")
